import { sameResource, remoteURL, safeSlice, validateResult } from './core.mjs';
...
export function usablePartialText(text) {
  ...
  const declaredWall = /\b(?:this|the) (?:article|content) is (?:(?:only )?for|(?:only )?available (?:only )?to) (?:paid )?(?:subscribers|members)\b|\b(?:this|the) (?:article|content) requires (?:an? )?(?:active )?(?:subscription|account)\b|(?:本文|本内容|此内容)仅(?:供|对)(?:付费)?(?:订阅者|会员)/i.test(clean);
  return clean.length >= 80 && !declaredWall && !/verify (?:that )?you are human|checking your browser|complete (?:the )?security check|access denied|captcha|subscribe to (?:continue|read|unlock)|subscription required|sign in to (?:continue|read)|log in to (?:continue|read)|验证您是人类|访问验证|验证码|订阅后(?:继续)?(?:阅读|查看)|登录后(?:继续)?(?:阅读|查看)/i.test(clean);
}

export async function anonymousFallback(job) {
  const target = remoteURL(job.url);
  ...
  try {
    ...
    if (!response.ok || !sameResource(response.url, job.url)) throw new Error(`匿名回退未取得原站正文（HTTP ${response.status}）`);
    ...
    if (/^text\/plain\b/i.test(type)) text = raw.trim();
      ...
      title = safeSlice((doc.querySelector('title')?.textContent || '').trim(), 1000);
      ...
      links = [...root.querySelectorAll('a[href]')].slice(0, 200).flatMap(node => {
        try {
          ...
          const label = safeSlice(node.textContent.trim(), 1000);
    ...
    text = safeSlice(text, job.max_chars);
    return validateResult({ url: response.url, title, text, markdown: safeSlice(`# ${title}\n\n${text}`, job.max_chars * 2), links, captured_at: new Date().toISOString(), truncated, degraded: true, quality: 'partial' }, job);
  } catch (error) {
