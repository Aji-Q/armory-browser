...
页面身份和任务身份分开：job id 在 Relay 持久化，tab id 仅在浏览器；读取前及回传时要求原任务 origin、路径和查询串匹配（仅忽略 fragment 和末尾斜杠）；不把同域的不同资源视为正文。跳到身份提供商时，不采集该登录页；用户完成后回到原文章再恢复；专属页同域错路由会在用户点击恢复后重新导航请求 URL。
